...
export default function SummarizeLayout() {
    ...
    // const filteredRole = (CraneSolutionReduer.result).solution_id === rolesReducer.result?.group
    // console.log(CraneSolutionReduer.result)
    ...
    return (
        <>
            {isLoading ? (
                <Loading />
            ) : ((CraneSolutionReduer.result)?.total_cost === 0 ? (
                    ...
                    <Box className='grid grid-cols-12 gap-x-5'>
                        <Box className='col-span-12 grid grid-cols-7 gap-x-5'>
                            ...
                            <SummarizaCard
                                title={'ต้นทุนรวม'}
                                price={
                                    ((CraneSolutionReduer.result)?.total_consumption_cost
                                        + (Solution_carrier_orderReducer.result.penalty_cost)
                                        + (CraneSolutionReduer.result)?.total_wage_cost)
                            ...
                            <SummarizaCard
                                title={'ค่าเชื้อเพลิง'}
                                price={(CraneSolutionReduer.result)?.total_consumption_cost.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                                icon={CurrencyBitcoinIcon}
                                unit={'บาท'}
                                color='bg-[#00a6fb]/50'
                            />
                            <SummarizaCard
                                title={'ค่าแรง'}
                                price={(CraneSolutionReduer.result)?.total_wage_cost.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                                icon={CurrencyBitcoinIcon}
                                unit={'บาท'}
                                color='bg-[#00a6fb]/50'
                            />
