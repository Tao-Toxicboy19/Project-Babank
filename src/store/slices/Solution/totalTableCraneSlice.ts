...
export const totalTableCranesV3Async = createAsyncThunk(
...
const totalTableCraneSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder.addCase(totalTableCranesV3Async.fulfilled, (state: TotalTableCraneState, action: PayloadAction<TotalTableCrane[]>) => {
            state.result = action.payload
            state.loading = false
            state.error = false
        });

        builder.addCase(totalTableCranesV3Async.rejected, (state: TotalTableCraneState) => {
            state.result = []
            state.loading = false
            state.error = true
        });

        builder.addCase(totalTableCranesV3Async.pending, (state: TotalTableCraneState) => {
            state.result = []
            state.loading = true
            state.error = false
        });
